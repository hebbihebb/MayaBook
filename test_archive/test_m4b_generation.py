#!/usr/bin/env python3
"""
Test M4B generation from partial WAV files generated during stress test

This script:
1. Concatenates the 817 partial WAV files into a single WAV
2. Converts to M4B audiobook format
3. Tests chapter metadata generation
...
# Configure logging
log_filename = f"m4b_test_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
logging.basicConfig(
    level=logging.INFO,
...
def main():
    print_header("M4B Generation Test from Stress Test Partial WAV Files")
    ...
    try:
        ...
        # Step 4: Create M4B audiobook
        print("\nStep 4: Creating M4B audiobook...")
        logger.info("Importing M4B export module...")
        from core.m4b_export import create_m4b_stream

        m4b_output = os.path.join(OUTPUT_DIR, "audiobook.m4b")
        logger.info(f"Creating M4B: {m4b_output}")

        m4b_path = create_m4b_stream(
            wav_path=concat_wav,
            m4b_path=m4b_output,
            metadata_title="MayaBook Stress Test - Partial Conversion",
            metadata_author="Test Suite",
            metadata_genre="Audiobook"
        )

        if not os.path.exists(m4b_path):
            raise RuntimeError(f"M4B creation failed - output not found: {m4b_path}")

        m4b_size_mb = os.path.getsize(m4b_path) / (1024 * 1024)
        logger.info(f"✓ M4B created successfully: {m4b_size_mb:.1f} MB")
        print(f"✓ M4B created: {m4b_path} ({m4b_size_mb:.1f} MB)")
        ...
        result = subprocess.run(
            ['ffprobe', '-v', 'error', '-show_format', '-show_streams', m4b_path],
            capture_output=True,
            text=True
        )
        ...
        if result.returncode == 0:
            ...
            logger.warning("ffprobe verification failed (may not be installed)")
        ...
        logger.info(f"M4B Output: {m4b_path} ({m4b_size_mb:.1f} MB)")
        logger.info("="*80)
